...
fn main() {
    ...
    for _ in 0..200 {
        loop {
            let frame = recorder.raw().get_next_pixel_buffer().expect("Error");
            ...
            //         // }
            //         println!(
            //             "Recieved BGRA frame - width = {}, height = {}, size = {}",
            //             frame.width,
            //             frame.height,
            //             frame.data.len(),
    ...
    //     let codec = avcodec_find_encoder_by_name(c!("h264_videotoolbox"));

    //     let stream = avformat_new_stream(output, null());
    //     (*stream).id = ((*output).nb_streams - 1) as i32;

    //     let encoder = &mut *avcodec_alloc_context3(codec);
    ...
    //     (*encoder).pix_fmt = AVPixelFormat::AV_PIX_FMT_NV12;
    ...
    //     av_dict_set(&mut opts, c!("preset"), c!("ultrafast"), 0);
    //     av_dict_set(&mut opts, c!("tune"), c!("zerolatency"), 0);
    //     avcodec_open2(encoder, codec, &mut opts);

    //     let bgra_frame = &mut *av_frame_alloc();
    ...
    //     let enc_frame = &mut *av_frame_alloc();
    //     enc_frame.format = AVPixelFormat::AV_PIX_FMT_NV12 as i32; //(*encoder).pix_fmt as i32;
    ...
    //         enc_frame.pts = i as i64 * 90000 / fps as i64;
    ...
    //     avcodec_free_context(&mut (encoder as *mut _));
